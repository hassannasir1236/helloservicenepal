        ...
        <div class="row vendor_payout_create">
            ...
            <div class="vendor_payout_create-inner">
                ...
                <fieldset>
                    ...
                    <div class="form-group row width-100">
                        <label class="col-3 control-label">{{trans('lang.section_position')}}</label>
                        <div class="col-7">
                            <input type="number" name="position" class="form-control position" id="position" min="1">
                            <div class="form-text text-muted">{{ trans("lang.section_position_help") }}</div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(".edit-setting-btn").click(function () {
            ...
            if (name == '') {
            ...
            } else if (isNaN(position) || position < 1) {
                $(".error_top").show();

                $(".error_top").html("");

                $(".error_top").append("<p>{{trans('lang.enter_valid_position')}}</p>");
                ...
                storeImageData().then(IMG => {
                    ...
                    swapSectionPositions(id, position);
    ...
    async function swapSectionPositions(id, newPosition) {
        if (!id || newPosition === null || newPosition === undefined) {
            console.error("❌ Error: Invalid ID or position.");
            return;
        }

        try {
            let sectionsRef = database.collection("sections");

            // 🔍 Get the current document
            let currentDoc = await sectionsRef.doc(id).get();
            if (!currentDoc.exists) {
                console.error("❌ Error: Document not found!");
                return;
            }

            let oldPosition = currentDoc.data().position;

            if (oldPosition === newPosition) {
                console.log("ℹ️ No change needed. Same position assigned.");
                return;
            }

            // 🔍 Find the document that currently has the newPosition
            let querySnapshot = await sectionsRef.where("position", "==", newPosition).get();

            let batch = database.batch();

            if (!querySnapshot.empty) {
                // 🛑 Swap positions: The document with `newPosition` gets `oldPosition`
                let otherDoc = querySnapshot.docs[0];
                batch.update(sectionsRef.doc(otherDoc.id), { position: oldPosition });
            }

            // ✅ Assign new position to the current document
            batch.update(sectionsRef.doc(id), { position: newPosition });

            await batch.commit();
            console.log(`✅ Swapped positions: ${id} (${oldPosition} → ${newPosition})`);
        } catch (error) {
